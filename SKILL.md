...
**この1文が承認されるまでフェーズ1には進まない。**

確定後、spec.md に「## 対象価値一致サマリ」セクションを先頭に書く。内容：承認された1文 + ①〜⑧の確定値テーブル。
...
## 5. 画像生成AI プロンプト集
### 5-1. 共通プロンプト（GPT-4o image / Nano Banana / SD の3系統）
### 5-2. 各スタンプ個別プロンプト（#01 〜 #N、3系統）
### 5-2.5. テキスト描画ルール
### 5-3. メイン画像・トークタブ画像
### 5-4. キャラシート（検収用）プロンプト
